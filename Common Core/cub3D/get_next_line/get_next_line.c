...
/*   Updated: 2026/07/18 18:56:12 by yamosca-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

static char	*trim_line(char *stock)
{
	char	*str;
	int		i;
	int		j;
	...
	j = i;
	while ((i + k < (int)ft_strlen(stock)) && str)
		str[k++] = stock[j++];
	free(stock);
	if (!*str || !str)
	{
...
static char	*result_line(char *stock)
{
	char	*result;
	int		i;
	char	tmp;
...
static char	*stock_line(char *stock, char *buffer)
{
	char	*stock_total;
	int		len1;
	int		len2;
...
static char	*read_file(int fd, int byte, char *buffer, char *stock)
{
	while (byte > 0)
	{
