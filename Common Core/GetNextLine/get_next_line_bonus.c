...
/*   Updated: 2025/11/09 18:41:35 by yamosca-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	byte = 1;
	buffer = NULL;
	stock[fd] = read_file(fd, byte, buffer, stock[fd]);
	if (stock[fd] && ft_is_in(stock[fd]))
	{
		result = result_line(stock[fd]);
		stock[fd] = trim_line(stock[fd]);
		return (result);
	}
	if (stock[fd] && *stock[fd])
	{
		result = ft_strdup(stock[fd]);
		free(stock[fd]);
		stock[fd] = NULL;
		return (result);
	}
